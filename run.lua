...
cmd:option("-snapshot_dir", "./snapshot_29/", "snapshot directory")
cmd:option("-snapshot_epoch", 1, "snapshot after how many iterations?")
...
function train_one_epoch(letters)
	...
	for mini_batch_start = 1,5000, batch_size do --for each mini-batch
		...
		for i = mini_batch_start, mini_batch_stop do 
			...
			a=choose_random_font()
			if not same then
				b=choose_random_font() 
				while b==a do
					b=choose_random_font() 
				end                     
			else
